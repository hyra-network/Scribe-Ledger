...
async fn main() -> Result<()> {
    ...
    let keys: Vec<Vec<u8>> = (0..test_size).map(|i| format!("key{}", i).into_bytes()).collect();
    let values: Vec<Vec<u8>> = (0..test_size).map(|i| format!("value{}", i).into_bytes()).collect();
    ...
    println!("  Sync flush: {:.0} ops/sec ({:.2} ms total)", 
            sync_ops_per_sec, sync_duration.as_secs_f64() * 1000.0);
    ...
    println!("  Async flush: {:.0} ops/sec ({:.2} ms total)", 
            async_ops_per_sec, async_duration.as_secs_f64() * 1000.0);
    ...
    println!("  Minimal flush: {:.0} ops/sec ({:.2} ms total)", 
            optimal_ops_per_sec, optimal_duration.as_secs_f64() * 1000.0);
    ...
    println!("Sync flush:      {:.0} ops/sec (baseline)", sync_ops_per_sec);
    println!("Async flush:     {:.0} ops/sec ({:+.1}% improvement)", async_ops_per_sec, sync_improvement);
    println!("Minimal flush:   {:.0} ops/sec ({:+.1}% improvement)", optimal_ops_per_sec, optimal_improvement);
